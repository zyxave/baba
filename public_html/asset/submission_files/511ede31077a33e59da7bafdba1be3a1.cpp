...
void makePascal(){
	...
	for (int i=2; i<26; i++){
		...
		for (int j=0; j<segitigaPascal[i].size(); j++){
			//printf("%d ", segitigaPascal[i][j]);
		}
		//printf("\n");
	}
}
...
int main(){
	...
	for (int i=0; i<testcase; i++){
		...
		for (int j=0; j<segitigaPascal[q].size(); j++){
			...
			//printf("koefisien %lld  pangkat %lld  segitigaPascal %lld\n", koefisien, pangkat(p, j), segitigaPascal[q][j]);
