...
class Registration extends MY_Controller
{
    ...
    {
        $this->data['pageTitle'] = 'ILPC UBAYA 2018 | Informatics Logical Programming Competition | Universitas Surabaya';
...
//        }
    ...
    public function register_teacher()
    {
        ...
        {
            $this->form_validation->set_rules('teachername', "Nama", 'trim|required|max_length[50]');
            $this->form_validation->set_rules('teacheremail', "Email", 'trim|required|max_length[100]|valid_email');
            $this->form_validation->set_rules('teachertelp', "No. Handphone", 'trim|required|max_length[15]|callback__valid_phone');
            $this->form_validation->set_rules('vegetarian', "Status Vegetarian", 'required');
            $this->form_validation->set_rules('teacherallergy', "Alergi Makanan", 'max_length[150]');
            $this->form_validation->set_rules('teacheraddress', "Alamat", 'trim|required|max_length[200]');
     ...
     public function register()
    {
        if($this->input->post('btnRegister') && isset($_SESSION['teacher_id']) && isset($_SESSION['teacher_id']))
        {
            ...
            $this->form_validation->set_rules('conf_password', "Confirm Password", 'required|matches[password]|min_length[8]');
            $cadangan = false;
            ...
            for ($i = 0; $i <= $jum_tim; $i++)
            {
                ...
                if($x == 4)
                {
                     $this->form_validation->set_rules("member_name_" . $i . "", "Nama Anggota Cadangan", 'trim|required|max_length[50]');
		    $this->form_validation->set_rules("member_phone_" . $i . "", "Nomor HP Anggota Cadangan", 'trim|required|numeric|max_length[15]|callback__valid_phone');
                    $this->form_validation->set_rules("member_email_" . $i . "", "Email Anggota Cadangan", 'required|max_length[100]|valid_email');
                    $this->form_validation->set_rules("member_grade_" . $i . "", "Kelas Anggota Cadangan", 'required');
                     $this->form_validation->set_rules("member_allergy_" . $i . "", "Alergi Anggota Cadangan", 'trim|max_length[150]');
                    $this->form_validation->set_rules("member_vegetarian_" . $i . "", "Status Vegetarian Anggota Cadangan", 'required');
                }
                else
                {
                    $this->form_validation->set_rules("member_name_" . $i . "", "Nama Anggota " . $x , 'trim|required|max_length[50]');
                     $this->form_validation->set_rules("member_phone_" . $i . "", "Nomor HP Anggota " . $x , 'trim|required|numeric|max_length[15]|callback__valid_phone');
                    $this->form_validation->set_rules("member_email_" . $i . "", "Email Anggota " . $x , 'required|max_length[100]|valid_email');
                    $this->form_validation->set_rules("member_grade_" . $i . "", "Kelas Anggota " . $x , 'required');
                     $this->form_validation->set_rules("member_allergy_" . $i . "", "Alergi Anggota " . $x , 'trim|max_length[150]');
            ...
            else // kalo data diisi lengkap, username & teamname tersedia
            {
                ...
                //kirim email ke kita setelah daftar
                $this->send_notif_registration($teamName); 
    ...
    private function send_notif_registration($teamName)
    {
        $sendgrid = new SendGrid('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');

        $textMessage = "Anda menerima email ini karena ada tim yang mendaftar

        Tim ".$teamName." baru saja melakukan pendaftaran
        ...
        Anda menerima email ini karena ada tim yang mendaftar

        Tim :"'.$teamName.'"" baru saja melakukan pendaftaran


        Salam,
        Sistem Informasi ILPC 2018
        ...
        $email
            ->addTo('ilpc.ubaya@gmail.com')
            ->setFrom('ilpc-ubaya@ifubaya.com')
            ->setSubject('[ILPC 2018] Tim Daftar')
            ->setText($textMessage)
            ->setHtml($textHtml);
        ;
        $sendgrid->send($email);
